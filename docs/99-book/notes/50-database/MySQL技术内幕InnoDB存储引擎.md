...
1. 体系结构和存储引擎
2. 文件 参数文件、日志文件、套接字文件、pid文件、表结构定义文件、InnoDB文件（表空间文件、重做日志文件）
3. 表 索引组织表、逻辑存储结构（表、段、区、页、行）、记录格式、数据页结构、约束、视图、分区表
4. 索引与算法 数据结构（二分查找、二叉查找树、平衡二叉树）、B+树、哈希算法、全文检索
5. 锁 lock与latch，锁的类型、锁的算法（Record、Gap、Next-key）、锁问题（脏读、不可重复读、丢失更新）、阻塞、死锁、锁升级
6. 事务 事务实现、事务控制语句、隔离级别、分布式事务（内部XA事务）、不好的事务习惯、长事务
7. 备份与恢复 冷备、逻辑备份、二进制日志备份与恢复、热备、快照备份、复制 **Backup-Recovery**
8. 性能优化 CPU、内存、硬盘、RAID、操作系统、文件系统、基准测试工具  **CMDN、OS-FS**

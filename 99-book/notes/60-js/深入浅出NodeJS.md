...
#### 3.2.2 理想的非阻塞异步I/O
perfect 应用程序发起非阻塞调用，无须通过遍历或者事件唤醒等方式轮询，可以直接处理下一个任务，只需在IO完成后通过信号
或者回调将数据传递给应用程序。
...
需要强调的一点是，这里的IO不仅仅只限于磁盘文件的读写。*nix将计算机抽象了一番，磁盘文件、硬件、套接字等几乎所有**计算机资源**都抽象为文件，因此这里描述的阻塞和非阻塞的情况同样适合于套接字等。
